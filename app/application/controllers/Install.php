...
//			redirect('install', 'refresh'); // um... you shouldn't be here... but let's give this error instead
        ...
        redirect('install/update_bamboo', 'refresh');
        exit('<strong>Not done yet!</strong> Now we need to update to the latest version of BambooInvoice.  Please follow this ' . anchor('install/update_bamboo',
                        'update link'));
    }
    // --------------------------------------------------------------------
    ...
    function update_bamboo()
    {
        ...
        if ($version == '0.8.0' OR $version == '0.8.1' OR $version == '0.8.2') {
            show_error('Updating beyond this point requires a newer version of BambooInvoice.  Please contact Cedric Perrot if you want some guidance migrating your data.');
        }
        ...
        if ($version == '0.8.3') {
            ...
            $this->dbforge->modify_column('invoice_items',
                    array('quantity' => array('name' => 'quantity', 'type' => 'DECIMAL', 'constraint' => '7,2', 'default' => 1)));
        ...
        if ($version == '0.8.9') {
            ...
            $field = array(
                ...
                'accounting_invoice_exchange_rate' => array(
                    'type' => 'DECIMAL',
                    'constraint' => '20,6',
                    'default' => 0),
            );
            ...
            $field = array(
                'currency_type_accounting' => array(
                    'type' => 'VARCHAR',
                    'constraint' => 20,
                    'default' => ''
                ),
                ...
                'exchange_surplus_middle_exchange_rate' => array(
                    'type' => 'VARCHAR',
                    'constraint' => 20,
                    'default' => ''
                ),
            ...
            $this->dbforge->modify_column('sessions', $field);
            //due to upgrade to CodeIgniter 2.2.x to host ip v6 addresses
            //ALTER TABLE bamboo_sessions CHANGE ip_address ip_address varchar(45) default '0' NOT NULL



            $this->db->set('bambooinvoice_version', '0.9.2');
            //$this->db->where('id', 1);
            $this->db->update('settings');

            $updates .= "<li>Upgrade to 0.9.2 success.</li>";
        }
        // regrab data, for the new update
        $version = $this->db->get('settings')->row()->bambooinvoice_version;
        if ($version == '0.9.2') {
            ...
            /* ALTER TABLE `bamboo_sessions` 
              CHANGE COLUMN `session_id` `id` VARCHAR(40) NOT NULL DEFAULT '0' ;
              ALTER TABLE `bamboo_sessions`
              CHANGE COLUMN `user_data` `data` TEXT NULL DEFAULT NULL ;
              ALTER TABLE `bamboo_sessions`
              CHANGE COLUMN `data` `data` BLOB NULL DEFAULT NULL ;
              ALTER TABLE `bamboo_sessions`
              CHANGE COLUMN `last_activity` `timestamp` INT(10) UNSIGNED NULL DEFAULT '0'; */

            $this->dbforge->modify_column('sessions', $field);

            /* ALTER TABLE `bamboo_sessions` DROP COLUMN `user_agent`; */
            try{
                $this->dbforge->drop_column('sessions', 'user_agent');
            }
            catch(Exception $e) {
                //probably dropped manually already
            }

            //due to upgrade to CodeIgniter 3.0.x and 3.1.2
            $field = array(
                'id' => array(
                    'type' => 'VARCHAR',
                    'constraint' => 128,
            ...
            $this->dbforge->modify_column('sessions', $field);
            $this->db->set('bambooinvoice_version', '1.0.0');
            ////$this->db->where('id', 1);
            $this->db->update('settings');
            $updates .= "<li>Upgrade to 1.0.0 success.</li>";
        }
        
        $version = $this->db->get('settings')->row()->bambooinvoice_version;
        if ($version == '1.0.0') {
            $this->db->set('bambooinvoice_version', '1.0.1');
            ////$this->db->where('id', 1);
            $this->db->update('settings');
            $updates .= "<li>Upgrade to 1.0.1 success.</li>";
        }
        ...
        if ($version == '1.0.1') {
            ...
            $this->dbforge->modify_column('clientcontacts', $fields);
            
            $this->db->set('bambooinvoice_version', '1.0.2');
            ////$this->db->where('id', 1);
            $this->db->update('settings');
            $updates .= "<li>Upgrade to 1.0.2 success.</li>";
        }
        
        $version = $this->db->get('settings')->row()->bambooinvoice_version;
        if ($version == '1.0.2') {
            $this->db->set('bambooinvoice_version', '1.0.3');
            ////$this->db->where('id', 1);
            $this->db->update('settings');
            $updates .= "<li>Upgrade to 1.0.3 success.</li>";
        }
        $version = $this->db->get('settings')->row()->bambooinvoice_version;
        if ($version == '1.0.3') {
            $fields = array(
                'display_zero_tax' => array(
                    'type' => 'CHAR',
                    'constraint' => 1,
                    'default' => 'n',
                ),
            ...
            $this->db->set('bambooinvoice_version', '1.0.4');
            ////$this->db->where('id', 1);
            $this->db->update('settings');
            $updates .= "<li>Upgrade to 1.0.4 success.</li>";
        }
          if ($version == '1.0.4') {
            
            $dbPrefix = $this->db->dbprefix;
            $sql = str_replace("#",$dbPrefix,"CREATE INDEX `client_id` ON `#_invoices`(`client_id`)");
            $this->db->query($sql);
            $sql = str_replace("#",$dbPrefix,"CREATE INDEX `invoice_number` ON `#_invoices`(`invoice_number`)");
            $this->db->query($sql);              
            $sql = str_replace("#",$dbPrefix,"CREATE INDEX `client_id` ON `#_clientcontacts`(`client_id`)");
            $this->db->query($sql);            
            $sql = str_replace("#",$dbPrefix,"CREATE INDEX `invoice_id` ON `#_invoice_histories`(`invoice_id`)");
            $this->db->query($sql);  
            $sql = str_replace("#",$dbPrefix,"CREATE INDEX `clientcontacts_id` ON `#_invoice_histories`(`clientcontacts_id`)");
            $this->db->query($sql);  
            $sql = str_replace("#",$dbPrefix,"CREATE INDEX `invoice_id` ON `#_invoice_items`(`invoice_id`)");
            $this->db->query($sql);  
            $sql = str_replace("#",$dbPrefix,"CREATE INDEX `invoice_id` ON `#_invoice_payments`(`invoice_id`)");
            $this->db->query($sql);  
            $sql = str_replace("#",$dbPrefix,"CREATE INDEX `primary_contact_email` ON `#_settings`(`primary_contact_email`)");
            $this->db->query($sql);  

            $this->db->set('bambooinvoice_version', '1.0.5');
            ////$this->db->where('id', 1);
            $this->db->update('settings');
            $updates .= "<li>Upgrade to 1.0.5 success.</li>";
        }
      $updates .= '</ul>';
        ...
        <p>Have a comment or request?  Try leaving a note on the <a href="http://forums.bambooinvoice.net"><?php echo $this->lang->line('bambooinvoice_logo'); ?> forums</a>, or if you want to contact me directly, always feel free to email me at info@bambooinvoice.net.  Tech support stuff is probably better on the forums though ;)</p>
        ...
        <p>Happy invoicing!<br /><em>Cedric Perrot</em></p>
    ...
    /*
      ...
      $this->db->set('id', 1);
      $this->db->set('client_id', 0);
      $this->db->set('email', 'info@bambooinvoice.net');
      ...
      'country' 				=> 'Canada',
      'postal_code' 			=> 'ABC123',
      'website' 				=> 'http://bambooinvoice.net',
      'primary_contact' 		=> 'Joe Bloe',
      'primary_contact_email'	=> 'info@bambooinvoice.net',
      'logo' 					=> '',
      'logo_pdf' 				=> '',
