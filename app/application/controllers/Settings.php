...
class Settings extends MY_Controller
{
    ...
    function index()
    {
        ...
        if ($this->validation->run() == FALSE) {
            ...
            $data['row'] = $this->settings_model->get_row();
            ...
            $data = array(
                ...
                'display_zero_tax' => $this->input->post('display_zero_tax'),
                ...
                'currency_type_accounting' => $this->input->post('currency_type_accounting'),
                'api_key' => $this->input->post('api_key'),
                'exchange_surplus_middle_exchange_rate' => $this->input->post('exchange_surplus_middle_exchange_rate'),
            );
            ...
            //get old email address to be able to change contactdetails
            $this->load->model('settings_model');
            $old_primary_contact_email = $this->settings_model->get_setting('primary_contact_email');
            // run the update
            ...
            if ($this->db->affected_rows() == 1 OR $update_settings === TRUE) {
                ...
                $id = $this->clientcontacts_model->get_contact_id($old_primary_contact_email);

                if ($this->input->post('primary_contact_email') != '' && $this->settings_model->get_setting('demo_flag') != 'y') {

                    $this->clientcontacts_model->email_change($id, $this->input->post('primary_contact_email'));
                }
                ...
                if ($this->input->post('password') != '' && $this->input->post('password') == $this->input->post('password_confirm') && $this->settings_model->get_setting('demo_flag') != 'y') {
                    ...
                    $this->clientcontacts_model->password_change($id, $this->input->post('password'));
                }



                $this->session->set_flashdata('status',
                        $this->lang->line('settings_modify_success') . ' ' . $extra_message);
            } else {
    ...
    function _validation()
    {
        ...
        $rules['password'] = "min_length[8]|max_length[250]";
        ...
        $rules['currency_type_accounting'] = "trim|prep_for_form|max_length[20]";

        $rules['api_key'] = "trim|prep_for_form|max_length[200]";

        $rules['exchange_surplus_middle_exchange_rate'] = "trim|prep_for_form|numeric|max_length[10]";
        ...
        $fields['currency_type_accounting'] = $this->lang->line('settings_currency_type_accounting');

        $fields['api_key'] = $this->lang->line('settings_api_key');

        $fields['exchange_surplus_middle_exchange_rate'] = $this->lang->line('settings_exchange_surplus_middle_exchange_rate');
