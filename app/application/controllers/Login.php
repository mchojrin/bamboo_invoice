...
class Login extends MY_Controller
{
    ...
    function index()
    {
        ...
        {
            $data['languages'][$key] = rtrim(ucfirst($key),"/");
        }
    ...
    function forgot_password()
    {
        ...
        if ($this->validation->run() == FALSE) {
            ...
            $customer_id = $this->clientcontacts_model->password_reset($email,
                    $random_passkey);
            ...
            if ($customer_id AND $this->settings_model->get_setting('demo_flag') != 'y') {
                ...
                $email_body .= '<p>' . $this->lang->line('login_password_reset_email2') . ' ' . anchor("login/confirm_password/{$customer_id}/{$random_passkey}",
                                site_url("login/confirm_password/{$customer_id}/{$random_passkey}")) . ".</p>";
                ...
                $senderInfo = $this->settings_model->getCompanyInfo();
                $this->email->to($email);
                $this->email->from($this->settings_model->get_setting('primary_contact_email'),
                        $this->settings_model->get_setting('primary_contact'));
    ...
    function confirm_password()
    {
        ...
        $email = $this->clientcontacts_model->password_confirm($customer_id,
                        $passkey)->row()->email;
        ...
        if ($email != FALSE) {
            ...
            if ($this->settings_model->get_setting('demo_flag') == 'y') {
                ...
                if ($this->clientcontacts_model->password_change($customer_id, $new_password)) {
                    $email_body = '<p>' . $this->lang->line('login_password_email1') . " <em>$new_password</em> " . $this->lang->line('login_password_email2') . ' ' . anchor('login',
                                    $this->lang->line('login_login')) . '.</p>';
