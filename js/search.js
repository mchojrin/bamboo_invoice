...
function getInvoices (elm) {
	...
	new Ajax.Request(base_url+'invoices/retrieveInvoices/', {postBody: Form.serialize($('filter')), onComplete: function(response) {
                                        showInvoices(response)}})
}
function showInvoices(response){
    if (response.responseText != '') {
            //invoiceArray = eval('(' + response.responseText + ')');
			//replace eval through JSON.parse because eval could potentially execute some code
			invoiceArray = JSON.parse(response.responseText);
...
function drawTable(invoiceArray) {
	...
	for(i = 0; i < invoiceArray['invoices'].length; i++) {
		...
		if (invoice_id.indexOf('monthbreak') > -1) {
            ...
            spanTag.className = "short_description"; 
            spanTag.innerHTML = invoiceArray['invoices'][i].short_description; 

			var container = document.createElement('td');
			var theLink = document.createElement('a');
			theLink.setAttribute ('href', base_url+'invoices/view/'+invoiceArray['invoices'][i].invoiceId);
			var theData = document.createTextNode(invoiceArray['invoices'][i].currency_symbol + " " + invoiceArray['invoices'][i].amount);
			theLink.appendChild(theData);
			container.appendChild(theLink);
			...
			var theLink = document.createElement('a');
			theLink.setAttribute ('href', base_url+'invoices/view/'+invoiceArray['invoices'][i].invoiceId);
			var statusText = invoiceArray['invoices'][i].status;
			//looking for any digit
			var r = /\d+/;	//same as var r = new RegExp("\d+");
			if(statusText.match(r)){
				// found a digit in the text create a span node
				var spanData = document.createElement("span"); 
				spanData.className = "error"; 
				spanData.innerHTML = statusText;
				theLink.appendChild(spanData);
			}
			else{
				var theData = document.createTextNode(statusText);
				theLink.appendChild(theData);
			}
