...
class Utilities extends MY_Controller
{
    ...
    function version_check()
    {
        ...
        if ($status == 'new') {
            ...
            $data['message'] = $this->lang->line('utilities_new_version_available') . anchor('http://bambooinvoice.net',
                            'http://bambooinvoice.net');
    ...
    function _export($status, $client, $type = 'xml')
    {
        ...
        $invoices = $this->invoices_model->getInvoicesAJAX($status, $client,
                $this->settings_model->get_setting('days_payment_due'));
    ...
    function php_info()
    {
        ...
        if ($this->settings_model->get_setting('demo_flag') == 'y') {
            ...
            $output = preg_replace("/<table(.*?)>/", "\n\n<table\\1 class=\"tableBorderNoBot\" cellspacing=\"0\">",
                    $output);
